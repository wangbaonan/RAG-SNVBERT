...
class BERTFoundationModel(nn.Module):
    ...
    def __init__(self, bert: BERTWithRAG):
        ...
        super().__init__()
        self.bert = bert
        self.hap_classifier = EnhancedHaplotypeClassifier(bert.dims)
        self.gt_classifier = GenotypeClassifier(2, 4)

    def forward(self, x):
        # Haplotype.
        hap_1_after, hap_2_after, hap_1_before, hap_2_before = self.bert.forward(x)
        hap_1 = self.hap_classifier.forward(hap_1_after, x['af'], x['af_p'])  # hap_1.shape == (batch, seq_len, emb_dim)
        hap_2 = self.hap_classifier.forward(hap_2_after, x['af'], x['af_p'])
