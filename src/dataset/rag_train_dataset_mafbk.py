...
class RAGTrainDataset(TrainDataset):
    def __init__(self, vocab, vcf, pos, panel, freq, window, 
        ...
        self._cached_ref_data = None          
        self._initial_build_done = False

        self._build_ref_data = build_ref_data
        self._ref_vcf_path = ref_vcf_path
        #if build_ref_data and ref_vcf_path:
        #    self._build_faiss_indexes(ref_vcf_path)

    def refresh_indices(self, epoch):
        """动态重建索引"""
        if not self._initial_build_done:
            if self._build_ref_data and self._ref_vcf_path:
                self._build_faiss_indexes(self._ref_vcf_path)
                self._initial_build_done = True
        else:
            current_ratio = self.mask_scheduler.get_mask_ratio(epoch)
            self.maf_mask_percentage = int(current_ratio * 100) 
            self._rebuild_faiss_indexes(self._ref_vcf_path)
            

    def _rebuild_faiss_indexes(self, ref_vcf_path):
        """优化后的重载索引方法"""
        self.ref_data_windows.clear()
        self.window_indexes.clear()
        self._build_faiss_indexes(ref_vcf_path)

    def _build_faiss_indexes(self, ref_vcf_path: str):
        """构建FAISS索引（完整EOS处理）"""
        maf_mask_percentage = self.maf_mask_percentage
        print(f"▣ 开始构建FAISS索引 (Mask比例={maf_mask_percentage}%)")
        ...
        for w_idx in tqdm(range(self.window_count), desc="处理窗口"):
            ...
            raw_mask = self.random_mask(current_pos, self.maf_mask_percentage)
            ...
            #print("raw_ref")
            #print(raw_ref.shape)
            self.raw_ref_data_windows.append(raw_ref) 
    ...
    def _load_ref_data(self, ref_vcf_path):
        """带缓存的参考数据加载"""
        if self._cached_ref_data is None:
            h5_path = os.path.splitext(ref_vcf_path)[0] + ".h5"
            if os.path.exists(h5_path):
                with h5py.File(h5_path, 'r') as f:
                    self._cached_ref_data = (f['calldata/GT'][:], f['variants/POS'][:])
            else:
                # 从VCF读取并保存到HDF5...
                callset = allel.read_vcf(ref_vcf_path, 
                               fields=['variants/POS', 'calldata/GT'],
                ...
                with h5py.File(h5_path, 'w') as h5f:
                    h5f.create_dataset('calldata/GT', data=ref_gt, compression='gzip')
                    h5f.create_dataset('variants/POS', data=ref_pos, compression='gzip')
                self._cached_ref_data = (ref_gt, ref_pos)
                pass
        return self._cached_ref_data
