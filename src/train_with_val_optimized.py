...
def main():
    ...
    parser.add_argument("--lr", type=float, default=5e-5, help="学习率 (优化: 5e-5)")
    parser.add_argument("--warmup_steps", type=int, default=10000, help="warmup步数")
    ...
    print(f"{'='*60}")
    print(f"Focal gamma: {args.focal_gamma} (原版: 5.0)")
    print(f"Use recon loss: {use_recon_loss} (原版: true)")
    print(f"Learning rate: {args.lr} (原版: 1e-5)")
    ...
    # 创建trainer (优化版)
    print(f"\n{'='*60}")
    print(f"Initializing Optimized Trainer...")
    print(f"{'='*60}")
