    ...
    def _core_infer_logic(self, dataloader: DataLoader) -> np.ndarray:
        ...
        with torch.cuda.amp.autocast(dtype=amp_dtype), torch.no_grad():
            for i, data in tqdm.tqdm(enumerate(dataloader), 
                ...
                # 异步数据传输
                gpu_data = {}
                for key in data_in_gpu:
                    if key in data:
                        tensor = data[key]
                        # 使用固定内存加速传输
                        if not tensor.is_pinned() and device.type == 'cuda':
                            tensor = tensor.pin_memory()
                        gpu_data[key] = tensor.to(device, non_blocking=True)
                
                # CUDA图优化（仅当输入形状稳定时）
                if static_input is None:
                    static_input = gpu_data
                    # 首次运行捕获计算图
                    with torch.cuda.stream(torch.cuda.Stream()):
                        static_output = self.model(gpu_data)
                    if device.type == 'cuda':
                ...
                for idx in range(sample_idx.shape[0]):
                    ...
                    # 使用内存视图避免临时拷贝
                    hap1_view = hap1_prob_mat[start:end, sample]
                    hap2_view = hap2_prob_mat[start:end, sample]
                    gt_view = gt_prob_mat[start:end, sample, :]
                    
                    hap1_view[:] = output[0][idx, 1:(end-start+1), 1].numpy()
                    hap2_view[:] = output[1][idx, 1:(end-start+1), 1].numpy()
                    gt_view[:] = output[2][idx, 1:(end-start+1), :].numpy()
