...
**重要**: 由于 Bug A 会导致 `window_valid_indices` 在预编码时初始化，**必须重新构建索引**。

```bash
# 清理旧索引（必须！因为 Bug A 修复改变了索引构建逻辑）
...
### 1. 必须清理索引
由于 Bug A 修复改变了索引构建逻辑（添加了 `window_valid_indices`），**旧索引不兼容**。
...
### 3. 预期训练时间
```
00:00 - 训练集预编码（40 分钟，构建 window_valid_indices）
00:40 - 验证集预编码（40 分钟）
...
### Q1: 为什么要清理索引？
**A**: Bug A 的修复在索引构建时添加了 `window_valid_indices` 的初始化。旧索引没有这个信息，会导致 `__getitem__` 无法正确过滤数据。
